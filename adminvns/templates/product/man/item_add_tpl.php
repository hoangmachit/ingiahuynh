...
function get_size($id=0,$type2='')
{
	global $d, $type;
	if($id)
	{
		$temps = $d->rawQueryOne("select id_$type2 from #_product where id = ? and type = ? limit 0,1",array($id,$type));
		$arr_size = explode(',', $temps["id_$type2"]);

		for($i=0;$i<count($arr_size);$i++) $temp[$i]=$arr_size[$i];
	}

$row_size = $d->rawQuery("select tenvi, id from #_product_size where type = ? and type2 = ? order by stt,id desc",array($type,$type2));

$str = '<select id="'.$type2.'_group" name="'.$type2.'_group[]" class="select multiselect" multiple="multiple" >';
		...
		<div class="row">
										...
										<div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
											...
											<?php if(isset($config['product'][$type]['motangan']) && $config['product'][$type]['motangan'] == true) { ?>
												<div class="form-group">
													...
													<input type="text" class="form-control for-seo" name="data[motangan<?=$k?>]" id="motangan<?=$k?>" placeholder="Mô tả ngắn (<?=$k?>)" value="<?=@$item['motangan'.$k]?>" >
											...
											<?php if(isset($config['product'][$type]['motangan2']) && $config['product'][$type]['motangan2'] == true) { ?>
												<div class="form-group">
													<label for="motangan2<?=$k?>">Mô tả dưới thành tiền (<?=$k?>):</label>
													<textarea class="form-control for-seo <?=(isset($config['product'][$type]['motangan2_cke']) && $config['product'][$type]['motangan2_cke'] == true)?'form-control-ckeditor':''?>" name="data[motangan2<?=$k?>]" id="motangan2<?=$k?>" rows="5" placeholder="Mô tả dưới thành tiền (<?=$k?>)"><?=htmlspecialchars_decode(@$item['motangan2'.$k])?></textarea>
												</div>
											<?php } ?>
			...
			<div class="<?=$colRight?>">
				<?php if(
					...
					<?php if(isset($config['product'][$type]['price']) && $config['product'][$type]['price'] == true) { ?>
					<div class="card card-primary card-outline text-sm">
						<div class="card-header">
							<h3 class="card-title">Danh mục <?=$config['product'][$type]['title_main']?></h3>
							<div class="card-tools">
							...
							<div class="form-group-category row">
								...
								<?php if(isset($config['product'][$type]['size']) && $config['product'][$type]['size'] == true) { ?>
									<div class="form-group col-12">
										<label class="d-block" for="id_size">(1) Danh mục kích thước:</label>
										<?=get_size(@$item['id'],'size')?>
									</div>
								<?php } ?>
								<?php if(isset($config['product'][$type]['chatlieu']) && $config['product'][$type]['chatlieu'] == true) { ?>
									<div class="form-group col-12">
										<label class="d-block" for="id_chatlieu">(2) Danh mục Chất liệu giấy:</label>
										<?=get_size(@$item['id'],'chatlieu')?>
									</div>
								<?php } ?>
								<?php if(isset($config['product'][$type]['somat']) && $config['product'][$type]['somat'] == true) { ?>
									<div class="form-group col-12">
										<label class="d-block" for="id_somat">(3) Danh mục Số mặt in:</label>
										<?=get_size(@$item['id'],'somat')?>
									</div>
								<?php } ?>
								<?php if(isset($config['product'][$type]['canmang']) && $config['product'][$type]['canmang'] == true) { ?>
									<div class="form-group col-12">
										<label class="d-block" for="id_canmang">(4) Danh mục Loại cán màng:</label>
										<?=get_size(@$item['id'],'canmang')?>
									</div>
								<?php } ?>
								<?php if(isset($config['product'][$type]['khoanlo']) && $config['product'][$type]['khoanlo'] == true) { ?>
									<div class="form-group col-12">
										<label class="d-block" for="id_khoanlo">(5) Danh mục Đóng số nhảy:</label>
										<?=get_size(@$item['id'],'khoanlo')?>
									</div>
								<?php } ?>
								<?php if(isset($config['product'][$type]['soduongcung']) && $config['product'][$type]['soduongcung'] == true) { ?>
									<div class="form-group col-12">
										<label class="d-block" for="id_soduongcung">(6) Danh mục Qui cách:</label>
										<?=get_size(@$item['id'],'soduongcung')?>
									</div>
								<?php } ?>
								<?php if(isset($config['product'][$type]['soluong']) && $config['product'][$type]['soluong'] == true) { ?>
									<div class="form-group col-12">
										<label class="d-block" for="id_soluong">(7) Danh mục Số lượng:</label>
										<?=get_size(@$item['id'],'soluong')?>
									</div>
								<?php } ?>
								<?php if(isset($config['product'][$type]['hinhdang']) && $config['product'][$type]['hinhdang'] == true) { ?>
									<div class="form-group col-12">
										<label class="d-block" for="id_hinhdang">(8) Danh mục Thời gian:</label>
										<?=get_size(@$item['id'],'hinhdang')?>
									</div>
								<?php } ?>
								<?php if(isset($config['product'][$type]['cachthuc']) && $config['product'][$type]['cachthuc'] == true) { ?>
									<div class="form-group col-12">
										<label class="d-block" for="id_cachthuc">(9) Danh mục Số lượng trang ruột:</label>
										<?=get_size(@$item['id'],'cachthuc')?>
									</div>
								<?php } ?>
								<?php if(isset($config['product'][$type]['kieube']) && $config['product'][$type]['kieube'] == true) { ?>
									<div class="form-group col-12">
										<label class="d-block" for="id_kieube">(10) Danh mục Số đường cấn:</label>
										<?=get_size(@$item['id'],'kieube')?>
									</div>
								<?php } ?>
				...
				<div class="row">
					<?php if(isset($config['product'][$type]['ma']) && $config['product'][$type]['ma'] == true) { ?>
						<div class="form-group col-md-4">
							...
							<input type="text" class="form-control" required name="data[masp]" id="masp" placeholder="Mã sản phẩm" value="<?=@$item['masp']?>">
						</div>
					<?php } ?>
					...
					<?php if(isset($config['product'][$type]['donvi']) && $config['product'][$type]['donvi'] == true) { ?>
						<div class="form-group col-md-4">
							<label class="d-block" for="donvi">Đơn vị tính:</label>
							<div class="input-group">
								<input type="text" class="form-control " name="data[donvi]" id="donvi" placeholder="Đơn vị tính" value="<?=@$item['donvi']?>">
							</div>
						</div>
					<?php } ?>
