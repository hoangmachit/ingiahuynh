...
{
	$cart->addtocart($quantity,$id,$mau,$size,$type);
...
else if($cmd == 'update-cart' && $id > 0 && $code != '')
{
	...
	$proinfo = $cart->get_product_info($id,$type);


	if($size > 0){

		$getprice_size_loai = $d->rawQueryOne("select gia,giamoi from #_product_price where id_size = ? and id_product = ? and type = ? and hienthi > 0 order by stt,id desc",array($size,$id,$type));

		if($getprice_size_loai['gia'] && $getprice_size_loai['giamoi']){
			$proinfo['gia'] = $getprice_size_loai['gia'];
			$proinfo['giamoi'] = $getprice_size_loai['giamoi'];
		}elseif($getprice_size_loai['giamoi']){
			$proinfo['giamoi'] = $getprice_size_loai['giamoi'];
		}elseif($getprice_size_loai['gia']){
			$proinfo['gia'] = $getprice_size_loai['gia'];
		}
				...
				<div class="top-cart">
					<div class="list-procart">
						...
						<?php if(isset($_SESSION['cart'])) { for($i=0;$i<count($_SESSION['cart']);$i++) {
							...
							$proinfo = $cart->get_product_info($pid,$type);
							if($size > 0){
								$getprice_size_loai = $d->rawQueryOne("select gia,giamoi from #_product_price where id_size = ? and id_product = ? and type = ? and hienthi > 0 order by stt,id desc",array($size,$pid,$type));
						...
						<div class="quantity-procart">
							...
							<div class="quantity-counter-procart quantity-counter-procart-<?=$code?> d-flex align-items-stretch justify-content-between">
								...
								<input type="number" class="quantity-procat" min="1" value="<?=$quantity?>" data-size="<?=$size?>" data-type="<?=$type?>" data-pid="<?=$pid?>" data-code="<?=$code?>"/>
