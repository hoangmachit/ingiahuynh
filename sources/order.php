...
/* Tỉnh thành */
$city = $d->rawQuery("select ten, id from #_city order by stt,id asc");
...
if(isset($_POST['thanhtoan']))
{
	...
	if($id_insert)
	{
		for($i=0;$i<$max;$i++)
		{
			...
			$id_size = $_SESSION['cart'][$i]['size'];
			if($id_size > 0){
				$ordergia =  $d->rawQueryOne("select gia, giamoi from #_product_price where id_product = ? and id_size = ? and type = ? and hienthi > 0 limit 0,1",array($pid,$id_size,$type));

				if($ordergia['gia'] && $ordergia['giamoi']){
					$gia = $ordergia['gia'];
					$giamoi = $ordergia['giamoi'];
				}elseif($ordergia['giamoi']){
					$giamoi = $ordergia['giamoi'];
				}elseif($ordergia['gia']){
					$gia = $ordergia['gia'];
				}
