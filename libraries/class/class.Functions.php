...
class Functions
{
	...
	public function get_thuonghieu($table='', $type='', $lang='vi', $title_select='Chọn thương hiệu', $class_select='select-category')
	{
		$id_parent = 'id_thuonghieu';

		$rows = $this->d->rawQuery("select ten$lang as ten, id, id_thuonghieu from #_".$table." where hienthi = 1 and type = '".$type."' order by stt asc,id desc");
	...
	/* Lấy size cart */
	public function get_size_cart($id=0)
	{
		if($id)
		{
			$row = $this->d->rawQueryOne("select tenvi from #_product_size where id = ? limit 0,1",array($id));
			return $row;
		}
		else
		{
	...
	/* Lấy size cart */
	public function get_name_cart($id=0)
	{
		if($id)
		{
			$row = $this->d->rawQueryOne("select tenvi from #_product_size where id = ? limit 0,1",array($id));
	...
	public function getImageProduct($v = array())
	{
		...
		$result .= '<img onerror="this.src=\'' . THUMBS . '/540x511x2/assets/images/noimage.png\'"src="' . WATERMARK . '/product/540x511x1/' . UPLOAD_PRODUCT_L . $v['photo'] . '" alt="' . $v['ten'] . '">';
		$result .= '</a>';
		$result .=	'</div>';
		return $result;
	}
